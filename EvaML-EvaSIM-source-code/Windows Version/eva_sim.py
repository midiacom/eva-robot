...
#import winsound # winsound substituindo a lib playsound que funcionou bem no linux
...
w = 1200
h = 550  # on linux use 525 # on windows use 550
...
frame_robot.pack(pady= 22, side=tkinter.LEFT) # , on linux use 5
...
def evaInit():
    ...
    playsound("my_sounds/power_on.wav")
    terminal.insert(INSERT, "\nstate: Speaking a greeting text.")
    playsound("my_sounds/greetings.wav")
    terminal.insert(INSERT, '\nstate: Speaking "Load a script file and enjoy."')
    playsound("my_sounds/load_a_script.wav")
    terminal.insert(INSERT, "\nstate: Entering in standby mode.")
    bt_import['state'] = NORMAL
    evaMatrix("white")
    # retirada a animacao pra nao dar problema, caso o programa seja encerrado aqui
...
bt_padx = 3 # ajuste de espaçamento entre botões
...
def exec_comando(node):
    ...
    if node.tag == "motion":
       ...
       time.sleep(1) # Um tempo apenas simbólico. No robô, o movimento não bloqueia o script e demorar tempos distintos
        ...
        if not (os.path.isfile("audio_cache_files/" + file_name + ".wav")): # se nao existe chama o watson
            # Eva tts functions
            with open("audio_cache_files/" + file_name + ".wav", 'wb') as audio_file:
                res = tts.synthesize(texto[ind_random], accept = 'audio/wav', voice = root.find("settings")[0].attrib["tone"]).get_result()
                audio_file.write(res.content)
        ledAnimation("SPEAK")
        print("audio_cache_files/" + file_name + ".wav")
        playsound("audio_cache_files/" + file_name + ".wav") # toca o audio da fala
        #time.sleep(1) # delay necessário na versao para windows, porque senão o block libera antes do tempo
        ledAnimation("STOP")
        ...
        try:
            ...
            #time.sleep(1) # necessario para a versão para windows
            ledAnimation("SPEAK")
